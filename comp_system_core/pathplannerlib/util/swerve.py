...
class SwerveSetpoint:
    robot_relative_speeds: ChassisVelocities
    module_states: list[SwerveModuleVelocity]
...
class SwerveSetpointGenerator:
    ...
    def generateSetpoint(self, prev_setpoint: SwerveSetpoint, desired_state_robot_relative: ChassisVelocities, dt: float,
        ...
        if constraints is not None:
            ...
            desired_state_robot_relative = ChassisVelocities(vel.x, vel.y, max(min(desired_state_robot_relative.omega,
        ...
        need_to_steer = True
        if self._epsilonEqualsSpeeds(desired_state_robot_relative, ChassisVelocities()):
            need_to_steer = False
            for m in range(self._config.numModules):
                desired_module_states[m].angle = prev_setpoint.module_states[m].angle
                desired_module_states[m].velocity = 0.0
        ...
        for m in range(self._config.numModules):
            prev_vx.append(
                ...
                * prev_setpoint.module_states[m].velocity
            )
            prev_vy.append(
            ...
            desired_vy.append(
                desired_module_states[m].angle.sin() * desired_module_states[m].velocity
            )
        ...
        if all_modules_should_flip \
                and not self._epsilonEqualsSpeeds(prev_setpoint.robot_relative_speeds, ChassisVelocities()) \
                and not self._epsilonEqualsSpeeds(desired_state_robot_relative, ChassisVelocities()):
            ...
            return self.generateSetpoint(prev_setpoint, ChassisVelocities(), dt, input_voltage, constraints)
        ...
        for m in range(self._config.numModules):
            ...
            if self._epsilonEquals(prev_setpoint.module_states[m].velocity, 0.0):
                ...
                if self._epsilonEquals(desired_module_states[m].velocity, 0.0):
            ...
            max_heading_change = \
                ...
                / ((self._config.massKG / self._config.numModules)
                   * math.fabs(prev_setpoint.module_states[m].velocity))
        ...
        for m in range(self._config.numModules):
            ...
            max_vel_step = math.fabs(accel_states[m].velocity * dt)
        ...
        ret_speeds = ret_speeds.discretize(dt)  # 2027: instance method; upstream Java assigns the result
        ...
        chassis_forces = ChassisVelocities(chassis_force_x, chassis_force_y, ang_torque)
