...
class FollowPathCommand(Command):
    ...
    _speedsSupplier: Callable[[], ChassisVelocities]
    _output: Callable[[ChassisVelocities, DriveFeedforwards], None]
    ...
    def __init__(self, path: PathPlannerPath, pose_supplier: Callable[[], Pose2d],
                 speeds_supplier: Callable[[], ChassisVelocities],
                 output: Callable[[ChassisVelocities, DriveFeedforwards], None],
    ...
    def end(self, interrupted: bool):
        ...
        if not interrupted and self._path.getGoalEndState().velocity < 0.1:
            self._output(ChassisVelocities(), DriveFeedforwards.zeros(self._robotConfig.numModules))
