...
class AutoShootAndPickup(commands2.SequentialCommandGroup):
    def __init__(self, container, indent=0) -> None:
        super().__init__()
        self.set_name(f'AutoShootAndPickup')
        ...
        self.add_commands(commands2.ParallelCommandGroup(
            commands2.WaitCommand(1).and_then(Intake_Deploy(intake=container.intake, position='down', indent=1)),
        ...
        self.add_commands(AutoToPoseClean(container=self.container, swerve=self.container.swerve, target_pose=None, mode="ball_pickup",control_type='not_pathplanner'))
