...
class BlockheadMech:
    ...
    def __init__(self):
        ...
        """
        # Dimensions
        self.width = 40  # inches (approx robot length + intake)
        self.height = 30 # inches (approx max height)
    ...
    def _init_drivetrain(self):
        ...
        self.root_chassis = self.mech_side.getRoot("chassis_root", self.start_x, 3.5)
        self.chassis_ligament = self.root_chassis.appendLigament(
            "chassis", 27, 0, 20, self.color_chassis
        )
        ...
        # Rear Wheel
        self.root_rear_wheel = self.mech_side.getRoot("rear_wheel", self.start_x + 5, 2)
        self.rear_wheel_ligament = self.root_rear_wheel.appendLigament(
            "rear_spoke", 2, 0, 6, self.color_wheel
        )
        self.rear_wheel_ligament_2 = self.root_rear_wheel.appendLigament(
            "rear_spoke_2", 2, 180, 6, self.color_wheel
        )
        
        # Front Wheel
        self.root_front_wheel = self.mech_side.getRoot("front_wheel", self.start_x + 22, 2)
        self.front_wheel_ligament = self.root_front_wheel.appendLigament(
            "front_spoke", 2, 0, 6, self.color_wheel
        )
        self.front_wheel_ligament_2 = self.root_front_wheel.appendLigament(
            "front_spoke_2", 2, 180, 6, self.color_wheel
        )
    ...
    def _init_intake(self):
        ...
        self.root_intake_mount = self.mech_side.getRoot("intake_mount", self.start_x + 27, 4.5)
        self.abs_intake_mount = 0 # Roots are 0
        
        # The vertical post
        self.abs_intake_post = 90
        self.intake_post = self.root_intake_mount.appendLigament(
            "intake_post", 6, self._get_rel_angle(self.abs_intake_post, self.abs_intake_mount), 6, self.color_intake
        )
        ...
        self.intake_spacer = self.intake_arm.appendLigament(
            "intake_spacer", 0.1, 180, 0, self.color_intake # relative angle 180 to go back up arm
        )
        self.intake_indicator = self.intake_spacer.appendLigament(
            "intake_indicator", 2, 90, 8, self.color_intake # 90 relative to spacer
        )
    ...
    def _init_hopper(self):
        ...
        self.root_hopper = self.mech_side.getRoot("hopper_root", self.start_x + 25, 5)
        self.abs_hopper = 180
        self.hopper_length = 16
        ...
        self.hopper_spacer = self.root_hopper.appendLigament(
            "hopper_spacer", 0.1, self._get_rel_angle(self.abs_hopper, 0), 0, self.color_hopper # weight 0 = invisible
        )
        self.hopper_indicator = self.hopper_spacer.appendLigament(
            "hopper_indicator", 2, self._get_rel_angle(90, self.abs_hopper), 6, self.color_hopper # 90 abs (Up)
        )
    ...
    def _init_indexer(self):
        ...
        self.indexer_spacer = self.hopper_tower.appendLigament(
            "indexer_spacer", 0.1, self._get_rel_angle(self.abs_indexer, self.abs_hopper), 0, self.color_indexer
        )
        self.indexer_indicator = self.indexer_spacer.appendLigament(
            "indexer_indicator", 2, -90, 8, self.color_indexer # -90 relative to indexer
        )
    ...
    def _init_shooter(self):
        ...
        self.shooter_hood = self.indexer_wheel.appendLigament(
            "shooter_hood", 8, self._get_rel_angle(self.abs_shooter, self.abs_indexer), self.line_weight, self.color_shooter
        )
        ...
        self.shooter_center_spacer = self.indexer_wheel.appendLigament(
            "shooter_center_spacer", 4, self._get_rel_angle(self.abs_shooter, self.abs_indexer), 0, self.color_shooter
        )
        self.flywheel_ligament = self.shooter_center_spacer.appendLigament(
            "flywheel", 2, 0, 6, self.color_wheel
        )
        self.flywheel_ligament_2 = self.shooter_center_spacer.appendLigament(
            "flywheel_2", 2, 180, 6, self.color_wheel
        )

    def _init_climber(self):
        """Stub: Extension arms."""
        # Attached to chassis center/back
        self.climber_root_y = 2
        self.root_climber = self.mech_side.getRoot("climber_root", self.start_x + 2, self.climber_root_y)
        self.abs_climber = 90
        self.climber_stage_1 = self.root_climber.appendLigament(
            "climber_stage_1", 15, self._get_rel_angle(self.abs_climber, 0), self.line_weight, self.color_climber
        )
        
        # Hook pointing left (180 abs)
        self.abs_hook = 180
        self.climber_hook = self.climber_stage_1.appendLigament(
            "climber_hook", 4, self._get_rel_angle(self.abs_hook, self.abs_climber), self.line_weight, self.color_wheel
        )
    ...
    def _init_ball(self):
        ...
        self.ball_root = self.mech_side.getRoot("ball_root", self.start_x + 40, 3)
        ...
        self.ball_spacer = self.ball_root.appendLigament(
            "ball_spacer", 3, -90, 0, self.color_ball
        )
        self.ball_ligament = self.ball_spacer.appendLigament(
            "ball_solid", 6, 180, 60, self.color_ball
        )
    ...
    def update_intake(self, deployed: bool, speed: float):
        ...
        if abs(speed) > 0:
            self.intake_anim_dist += abs(speed) * 0.2
    ...
    def update_hopper(self, speed: float):
        ...
        if abs(speed) > 0:
            self.hopper_anim_dist += speed * 0.2  # Scale speed for animation
    ...
    def update_indexer(self, speed: float):
        ...
        if abs(speed) > 0:
            self.indexer_anim_dist += speed * 0.1
